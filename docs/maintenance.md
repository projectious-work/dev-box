...
```bash
./scripts/maintain.sh release 0.3.0
```
...
```bash
./scripts/build-macos.sh 0.3.0
```

Attach additional binaries to the release after creation:

```bash
gh release upload v0.3.0 dist/dev-box-v0.3.0-x86_64-apple-darwin.tar.gz
```
...
```bash
./scripts/maintain.sh push-images 0.3.0
```

This pushes both versioned tags (`python-v0.3.0`) and `latest` tags (`python-latest`)
for each flavor.
...
- [ ] `podman pull ghcr.io/projectious-work/dev-box:base-v0.3.0` succeeds
...
```bash
podman build -t ghcr.io/projectious-work/dev-box:python-v0.3.0 images/python/
```
