...
**Status:** pending
**aibox CLI:** v0.19.2 → v0.20.0
**processkit:** v0.22.0

## Summary

aibox has been updated from v0.19.2 to v0.20.0. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.19.2 and v0.20.0 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.20.0** (processkit v0.22.0): Minor release: install integrity check + self-heal (`aibox doctor --integrity`, `context/.processkit-provenance.toml`, `SyncDecision::Reinstall`); preauth merge into committed `.claude/settings.json` (closes aibox#55, consumes processkit v0.22.0 `skill-gate/assets/preauth.json`); `--no-container` scaffold mode + `AIBOX_NO_CONTAINER` env var on init/sync; new Tier 1 E2E harness (`no_container_harness`, `preauth_merge`); slash-command name collisions hard-fail; content-diff classifies upstream-removed-stale skills; `mcp_config_hash` renamed `processkit_install_hash` and broadened to cover skills/schemas/processes/state-machines; permissions JSON shape bug (`permissions.allow` flat-key) fixed with nested merge preserving user entries; `aibox.toml` template ships commented `[mcp.permissions]` advisory; tolerates upstream PROVENANCE.toml stamping bug in v0.22.0 via tracing::warn!; all 709 tests passing.
  <https://github.com/projectious-work/aibox/releases/tag/v0.20.0>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox sync` was run for v0.20.0 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.20.0` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.20.0"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.22.0/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.22.0/AGENTS.md context/templates/processkit/v0.22.0/AGENTS.md`
...
processkit is pinned to `v0.22.0` (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.22.0`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.20.0.
