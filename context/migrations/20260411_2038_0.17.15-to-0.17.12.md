...
**Status:** pending
**aibox CLI:** v0.17.15 → v0.17.12
**processkit:** vlatest

## Summary

aibox has been updated from v0.17.15 to v0.17.12. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Review the release notes for each version in this upgrade:
- https://github.com/projectious-work/aibox/releases/tag/v0.17.12

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox sync` was run for v0.17.12 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.17.12` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.17.12"`
...
- [ ] Verify `/workspace/context/templates/processkit/vlatest/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.13.0/AGENTS.md context/templates/processkit/vlatest/AGENTS.md`
...
processkit is pinned to `vlatest` (upgraded from `v0.13.0` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.17.12.
