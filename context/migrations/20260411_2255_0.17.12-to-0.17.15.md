...
**Status:** pending
**aibox CLI:** v0.17.12 → v0.17.15
**processkit:** v0.13.0

## Summary

aibox has been updated from v0.17.12 to v0.17.15. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Review the release notes for each version in this upgrade:
- https://github.com/projectious-work/aibox/releases/tag/v0.17.13
- https://github.com/projectious-work/aibox/releases/tag/v0.17.14
- https://github.com/projectious-work/aibox/releases/tag/v0.17.15

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox sync` was run for v0.17.15 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.17.15` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.17.15"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.13.0/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.13.0/AGENTS.md context/templates/processkit/v0.13.0/AGENTS.md`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.17.15.
