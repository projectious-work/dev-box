...
### 2.48 Human identity resolution

5-level identity cascade:
1. Environment variable: `AIBOX_USER=bob@company.com`
2. AI provider identity (Claude account, GitHub user, etc.)
3. Git config: `git config user.email`
4. Explicit command: `aibox whoami`
5. Agent asks: "Who am I working with today?"

Multi-human repos: Actor files contain non-sensitive info (name, expertise, roles).
Private preferences in `~/.aibox/personal-profile.yaml` (never committed).
Small teams share a repo; larger teams use fork+PR.
...
42. **SQLite index from init**: Created by `aibox init`, not deferred. Queries work
    from first session.
43. **Human identity cascade**: env var → provider → git config → aibox whoami → ask.
    Private preferences in ~/.aibox/ (not committed). Actor files non-sensitive only.
