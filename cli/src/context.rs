...
pub(crate) fn update_gitignore(addons: &AddonsSection) -> Result<()> {
    ...
    // Regenerated by `aibox sync` for every developer from their own config.
    content.push_str(".mcp.json\n");
    ...
    content.push_str(".DocumentRevisions-V100\n");
...
fn ensure_aibox_entries(gitignore_path: &Path) -> Result<()> {
    let required_entries = [
        ...
        "context/private/",
        "context/**/private/",
        // Generated MCP client configs — merge team + personal server
        // definitions and must never be committed.
        ".mcp.json",
        ".cursor/mcp.json",
...
mod tests {
    ...
    #[test]
    #[serial]
    fn scaffold_creates_context_dir_and_gitignore() {
        in_temp_dir(|| {
            let mut config = crate::config::test_config();
